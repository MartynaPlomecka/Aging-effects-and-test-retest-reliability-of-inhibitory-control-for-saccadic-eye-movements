...
#best
lmreactioninter <- brm(gain~ 1 + typ*age + (1|sbj_id), 
                       data = koncowa)
...
datadoplota_all = aggregate(gain ~ age + typ + sbj_id, koncowa, mean) 
...
br = ggplot(df_plot, aes(x = age, y = Estimate)) +
  geom_violin(data=datadoplotapro, aes(x=age, y=gain), alpha=0.4, color="coral3", fill='coral1', width = 0.45) +
  geom_jitter(data=datadoplotapro, aes(x=age, y=gain), color="coral3", alpha=0.4, size=1.4, position = position_jitter(width = 0.45)) +
...
ar= ggplot(df_plotanti, aes(x = age, y = Estimate)) +
  geom_violin(data=datadoplota, aes(x=age, y=gain), alpha=0.4, color="lightblue3", fill='lightblue1', width = 0.45) +
  geom_jitter(data=datadoplota, aes(x=age, y=gain), color="lightblue3", alpha=0.4, size=1.4, position = position_jitter(width = 0.45)) +
  geom_errorbar(aes(ymin=Q2.5, ymax=Q97.5), position=position_dodge(), size=0.6, width=.4) +
  geom_point(shape=21, size=4, fill='grey77') +
  #geom_hline(yintercept=50, linetype=2) +
  xlab("") +
  ylab('antisaccades, gain') +
  ylim(0.5,1) +
  scale_x_discrete(breaks=c("0","1"),
...
eneuro_gain = grid.arrange(ar, br, nrow = 1,
                      top=textGrob("gain",gp=gpar(fontsize=20,fontface='bold')))
