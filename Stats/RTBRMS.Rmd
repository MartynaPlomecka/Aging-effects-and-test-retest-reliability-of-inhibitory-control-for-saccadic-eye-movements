...
datadoplota_all = aggregate(rt ~ age + typ + sbj_id, koncowa, mean) 
...
br = ggplot(df_plot, aes(x = age, y = Estimate)) +
  geom_violin(data=datadoplotapro, aes(x=age, y=rt), alpha=0.4, color="coral3", fill='coral1', width = 0.45) +
  geom_jitter(data=datadoplotapro, aes(x=age, y=rt), color="coral3", alpha=0.4, size=1.4, position = position_jitter(width = 0.45)) +
...
ar= ggplot(df_plotanti, aes(x = age, y = Estimate)) +
  geom_violin(data=datadoplota, aes(x=age, y=rt), alpha=0.4, color="lightblue3", fill='lightblue1', width = 0.45) +
  geom_jitter(data=datadoplota, aes(x=age, y=rt), color="lightblue3", alpha=0.4, size=1.4, position = position_jitter(width = 0.45)) +
  geom_errorbar(aes(ymin=Q2.5, ymax=Q97.5), position=position_dodge(), size=0.6, width=.4) +
  geom_point(shape=21, size=4, fill='grey77') +
  #geom_hline(yintercept=50, linetype=2) +
  xlab("") +
...
eneuro = grid.arrange(ar, br, nrow = 1,
                      top=textGrob("Reaction time",gp=gpar(fontsize=20,fontface='bold')))
